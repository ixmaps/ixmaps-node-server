...
var parser, isWin = (/^win/.test(require('os').platform())), traceCommand = 'traceroute', traceArgs = '-n -q 4';
  ...
  dns.lookup(host, function (err, address) {
    if (err && net.isIP(host) === 0) {
      ...
      spawned.stdout.on('data', function(data) {
        if (reportCB) {
          reportCB(null, { traceID: details.traceID, now: new Date(), host: host, data: data.toString(), buffer: buf, hops: parser.parse(buf)});
        }
        buf += data.toString();
      }).on('end', function(data) {
        cb(null, parser.parse(buf));
